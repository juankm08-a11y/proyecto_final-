...
async function start() {
    ...
    try {
        ...
        console.log("Kafka producer conectado a",KAKFA_BROKER)
    } catch (error) {
...
async function loopCreateMatches(producer, exchange) {
    while(running) {
        try {
            ...
            const delayedMsg = {matchId, teams, timestamp: Date.now(),scheduledAfterMs: DELAY_MS}

            const headers = {"x-delay":DELAY_MS}
            
            exchange.publish(DELAY_ROUTING_KEY, JSON.stringify(delayedMsg),{headers, contentType: "application/json"} )
            console.log(`Mensaje diferido publicado en RabbitMQ (exchange=${DELAY_EXCHANGE}, delay=${DELAY_MS}ms)->`,delayedMsg)
        } catch (error) {
            console.error("Error publicando MATCH_CREATED",err)
        }
...
start().catch((err) => {
    console.error("Fallo iniciando match_generando",err);
    process.exit(1)
})